...
ALL_OBJECT_TYPE = set(OBJECTS_TYPE) | set(RECEPACLE_TYPE)

class ai2thor(data.Dataset):
  num_classes = 110
  ...
  def __init__(self, opt, split):
    ...
    self.data_dir = os.path.join(opt.data_dir, 'ai2thor')
    ...
    self.annotations = list(sorted(os.listdir(self.annot_path)))
    ...
    print('==> initializing ai2thor data.'.format(split))
    self.num_samples = len(self.images)

    print('Loaded {} {} samples'.format(split, self.num_samples))

  # def __getitem__(self, idx):
  #   annotation_path = os.path.join(self.annot_path, self.annotations[idx])
